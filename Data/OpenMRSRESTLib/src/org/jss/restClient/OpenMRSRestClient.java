...
public class OpenMRSRestClient implements RestClient {
    ...
    private JSONArray getUriArray(String uri)
        ...
        cr.setChallengeResponse(ChallengeScheme.HTTP_BASIC, username, password);
        Representation entity = cr.get();
        //System.out.println(new JsonRepresentation(entity).getJsonObject().getJSONArray("results"));
        return (new JsonRepresentation(entity).getJsonObject().getJSONArray("results"));
    }
    ...
    public ArrayList<JSONObject> getAllObjects(String object)
            throws IOException, JSONException {
        ArrayList<JSONObject> result = new ArrayList<JSONObject>();
        JSONArray ja = getUri(baseUri + object).getJSONArray("results");
        if (ja != null) {
            for (int x = 0; x < ja.length(); x++) {
                result.add(ja.getJSONObject(x));
            }
