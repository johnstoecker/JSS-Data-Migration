...
public class JSSCancerRegistrationMigration {
    ...
    public static void main(String[] args) throws IOException, JSONException {
        JSSCancerRegistrationMigration crm = new JSSCancerRegistrationMigration(new OpenMRSDataClient(new OpenMRSRestClient("http://localhost:8080/openmrs/", "admin", "Hello123"), true),
                            ...
                            for (Map.Entry<Integer, String> e : headers.entrySet()) {
                                if (cellContainsValue(row.getCell(e.getKey()))) {
                                    ...
                                    if (e.getValue().equals("CANCER CODE")) {
                                        ...
                                        if (!value.isEmpty()) {
                                           // client.addCodedRegistrationObservation(patientUuid, registrationDate, ganUuid, e.getValue(), value);
                                        }
                                    } else {
                                        if (!value.isEmpty()) {
                                           // client.addRegistrationObservation(patientUuid, registrationDate, ganUuid, e.getValue(), value);
                                        }
                                    }
                                }
                            }
